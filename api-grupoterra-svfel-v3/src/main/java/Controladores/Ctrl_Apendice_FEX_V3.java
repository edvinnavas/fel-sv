...
public class Ctrl_Apendice_FEX_V3 implements Serializable {
    ...
    public String extraer_apendice_jde_fex_v3(Long id_dte, String ambiente, String DOCO_JDE, String DCTO_JDE, String MCU_JDE, Connection conn) {
        ...
        try {
            ...
            String CODIGO_CLIENTE = ctrl_base_datos.ObtenerString("SELECT F.AN8_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            String NUMERO_ORDEN = ctrl_base_datos.ObtenerString("SELECT F.DCTO_JDE || '-' || F.DOCO_JDE FROM DTE_FEX_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
