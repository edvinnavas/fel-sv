...
public class Ctrl_Apendice_ND_V3 implements Serializable {
    ...
    public String extraer_apendice_jde_nd_v3(Long id_dte, String ambiente, String DOCO_JDE, String DCTO_JDE, String MCU_JDE, Connection conn) {
        ...
        try {
            ...
            String CODIGO_CLIENTE = ctrl_base_datos.ObtenerString("SELECT F.AN8_JDE FROM DTE_ND_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            String NUMERO_ORDEN = ctrl_base_datos.ObtenerString("SELECT F.DCTO_JDE || '-' || F.DOCO_JDE FROM DTE_ND_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            String NUMERO_DOCUMENTO = ctrl_base_datos.ObtenerString("SELECT F.DCT_JDE || '-' || F.DOC_JDE FROM DTE_ND_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            String FECHA_VENCIMIENTO = ctrl_base_datos.ObtenerString("SELECT C16.VALOR || ' - ' || (SELECT NVL(FELSV_TO_NUMBER(F.AITRAR),0) FROM " + esquema + ".F03012@" + dblink + " F WHERE F.AIAN8=DTE.AN8_JDE) || ' días' || ' - ' || TO_CHAR(IDE.FECHA_HORA_EMISION + (SELECT NVL(FELSV_TO_NUMBER(F.AITRAR),0) FROM " + esquema + ".F03012@" + dblink + " F WHERE F.AIAN8=DTE.AN8_JDE), 'DD-MM-YYYY') infoCondicionOperacion FROM RESUMEN_ND_V3 RES LEFT JOIN CAT_016 C16 ON (RES.ID_CAT_016=C16.ID_CAT) LEFT JOIN IDENTIFICACION_ND_V3 IDE ON (IDE.ID_DTE=RES.ID_DTE) LEFT JOIN DTE_ND_V3 DTE ON (IDE.ID_DTE=DTE.ID_DTE) WHERE RES.ID_DTE=" + ID_DTE, conn);
            String CODIGO_DESTINO = ctrl_base_datos.ObtenerString("SELECT F.SHAN_JDE FROM DTE_ND_V3 F WHERE F.ID_DTE=" + ID_DTE, conn);
            String ENVIAR_A = ctrl_base_datos.ObtenerString("SELECT SHI.NOMBRE || ' ' || SHI.DIRECCION_COMPLEMENTO infoEnviarANombre FROM SHIPTO_ND_V3 SHI WHERE SHI.ID_DTE=" + ID_DTE, conn);
