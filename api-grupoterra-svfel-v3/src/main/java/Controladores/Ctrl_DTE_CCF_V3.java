...
public class Ctrl_DTE_CCF_V3 implements Serializable {
    ...
    public DTE_CCF_V3 generar_json_dte_ccf_v3(String ambiente, Long id_dte) {
        ...
        try {
            ...
            Ctrl_Receptor_CCF_V3 ctrl_receptor_ccf_v3 = new Ctrl_Receptor_CCF_V3();
            resultado.setReceptor(ctrl_receptor_ccf_v3.obtener_receptor_ccf_v3(id_dte, ambiente, conn));
    ...
    public void registro_db_respuesta_mh(String ambiente, RESPUESTA_RECEPCIONDTE_MH respuesta_recepciondte_mh, Long id_dte) {
        ...
        try {
            ...
            if (respuesta_recepciondte_mh.getCodigoMsg().trim().equals("001") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("002") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("CCC")) {
                ...
                File TargetFile;
                if (ambiente.equals("PY")) {
                     inputstream = cliente_rest_jasper.reporte_ccf_pdf(id_dte.toString());
                     TargetFile = new File("/FELSV3/pdf/felsv_ccf_" + id_dte + ".pdf");
                } else {
                    inputstream = cliente_rest_jasper.reporte_ccf_pdf_prod(id_dte.toString());
                    TargetFile = new File("/FELSV3/pdf_pd/felsv_ccf_" + id_dte + ".pdf");
                }
    ...
    public Long obtener_id_dte_codigo_generacion(String ambiente, String codigogeneracion) {
        ...
        try {
            ...
            conn.setAutoCommit(false);
            resultado = ctrl_base_datos.ObtenerLong("SELECT F.ID_DTE FROM DTE_CCF_V3 F WHERE F.RESPONSE_ESTADO='EMITIDO-CONTIGENCIA' AND F.RESPONSE_CODIGOGENERACION='" + codigogeneracion + "'", conn);
            conn.commit();
            conn.setAutoCommit(true);
        } catch (Exception ex) {
            try {
