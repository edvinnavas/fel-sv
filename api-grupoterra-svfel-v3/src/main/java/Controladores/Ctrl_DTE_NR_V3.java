...
public class Ctrl_DTE_NR_V3 implements Serializable {
    ...
    public void registro_db_respuesta_mh(String ambiente, RESPUESTA_RECEPCIONDTE_MH respuesta_recepciondte_mh, Long id_dte) {
        ...
        try {
            ...
            if (respuesta_recepciondte_mh.getCodigoMsg().trim().equals("001") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("002") || respuesta_recepciondte_mh.getCodigoMsg().trim().equals("CCC")) {
                ...
                File TargetFile;
                if (ambiente.equals("PY")) {
                    inputstream = cliente_rest_jasper.reporte_nr_pdf(id_dte.toString());
                    TargetFile = new File("/FELSV3/pdf/felsv_nr_" + id_dte + ".pdf");
                } else {
                    inputstream = cliente_rest_jasper.reporte_nr_pdf_prod(id_dte.toString());
                    TargetFile = new File("/FELSV3/pdf_pd/felsv_nr_" + id_dte + ".pdf");
                }
                ...
                File TargetFileJson;
                if (ambiente.equals("PY")) {
                    TargetFileJson = new File("/FELSV3/json/jsondte_nr_" + id_dte + ".json");
                } else {
                    TargetFileJson = new File("/FELSV3/json_pd/jsondte_nr_" + id_dte + ".json");
                }
    ...
    public Long obtener_id_dte_codigo_generacion(String ambiente, String codigogeneracion) {
        ...
        try {
            ...
            conn.setAutoCommit(false);
            resultado = ctrl_base_datos.ObtenerLong("SELECT F.ID_DTE FROM DTE_NR_V3 F WHERE F.RESPONSE_ESTADO='EMITIDO-CONTIGENCIA' AND F.RESPONSE_CODIGOGENERACION='" + codigogeneracion + "'", conn);
            conn.commit();
            conn.setAutoCommit(true);
        } catch (Exception ex) {
            try {
    ...
    public void registro_db_respuesta_infile(String ambiente, RESPUESTA_RECEPCIONDTE_INFILE respuesta_recepciondte_infile, Long id_dte) {
        ...
        try {
            ...
            String DOC_JDE = ctrl_base_datos.ObtenerString("SELECT F.DOC_JDE FROM DTE_NR_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String DCT_JDE = ctrl_base_datos.ObtenerString("SELECT F.DCT_JDE FROM DTE_NR_V3 F WHERE F.ID_DTE=" + id_dte, conn);
            String AEXP_JDE = ctrl_base_datos.ObtenerString("SELECT REPLACE(TO_CHAR(F.TOTALPAGAR,'9999999999D99MI'),'.','') AEXP_JDE FROM RESUMEN_NR_V3 F WHERE F.ID_DTE=" + id_dte, conn);
