...
public class Ctrl_Extension_NC_V3 implements Serializable {
    ...
    public String extraer_extension_jde_nc_v3(Long id_dte, String ambiente, String AN8_JDE, Connection conn) {
        ...
        try {
            ...
            String NOMBENTREGA = ctrl_base_datos.ObtenerString("SELECT F.NOMBENTREGA FROM EMISOR_ESTABLECIMIENTO_V3 F WHERE F.CODPUNTOVENTA IN (SELECT F.MCU_JDE FROM DTE_NC_V3 F WHERE F.ID_DTE=" + ID_DTE + ")", conn);
            String DOCUENTREGA = ctrl_base_datos.ObtenerString("SELECT F.DOCUENTREGA FROM EMISOR_ESTABLECIMIENTO_V3 F WHERE F.CODPUNTOVENTA IN (SELECT F.MCU_JDE FROM DTE_NC_V3 F WHERE F.ID_DTE=" + ID_DTE + ")", conn);
