...
public class Ctrl_DTE_CR_V3 implements Serializable {
    ...
    public Long obtener_id_dte_codigo_generacion(String ambiente, String codigogeneracion) {
        ...
        try {
            ...
            conn.setAutoCommit(false);
            resultado = ctrl_base_datos.ObtenerLong("SELECT F.ID_DTE FROM DTE_CR_V3 F WHERE F.RESPONSE_ESTADO='EMITIDO-CONTIGENCIA' AND F.RESPONSE_CODIGOGENERACION='" + codigogeneracion + "'", conn);
            conn.commit();
            conn.setAutoCommit(true);
        } catch (Exception ex) {
            try {
